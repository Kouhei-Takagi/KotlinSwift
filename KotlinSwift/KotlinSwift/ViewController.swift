...
class ViewController: NSViewController {
        ...
        oPanel.beginSheetModal(for: self.view.window!, completionHandler: { (button: NSApplication.ModalResponse) -> Void in
            if button == NSApplication.ModalResponse.OK {
                ...
                if let data = fileHandle?.readDataToEndOfFile() {
                    self.kotlinCode1.textStorage?.beginEditing()
                    self.kotlinCode1.textColor = NSColor.black
                    self.kotlinCode1.string = String(data: data, encoding: .utf8) ?? ""
                    self.kotlinCode1.textStorage?.endEditing()
    ...
    @IBAction func format(_ sender: NSButton) {
        let kotlinCodeString:String = kotlinCode1.string

        //Simple line replacements are here.
        swiftCodeString = kotlinCodeString.replacingOccurrences(of: "val ", with: "let ")
        swiftCodeString = swiftCodeString.replacingOccurrences(of: "println", with: "print")
        swiftCodeString = swiftCodeString.replacingOccurrences(of: "fun ", with: "func ")
        swiftCodeString = swiftCodeString.replacingOccurrences(of: ":", with: "->")
        swiftCodeString = swiftCodeString.replacingOccurrences(of: "${", with: "\\(")
        swiftCodeString = swiftCodeString.replacingOccurrences(of: "for (i in 0..<count - 1)", with: "for i in 0..count")
        swiftCodeString = swiftCodeString.replacingOccurrences(of: "arrayOf<String>()", with: "[String]()")
        swiftCodeString = swiftCodeString.replacingOccurrences(of: "mapOf<String, Float>()", with: "String: Float]()")
        swiftCodeString = swiftCodeString.replacingOccurrences(of: "..", with: "...")
        swiftCodeString = swiftCodeString.replacingOccurrences(of: "until", with: "..<")
        swiftCodeString = swiftCodeString.replacingOccurrences(of: "when ", with: "switch ")
        swiftCodeString = swiftCodeString.replacingOccurrences(of: "arrayOf(", with: "[")
